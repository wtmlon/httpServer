...
void HttpData::handleRead() {
	cout<<"Http::HandleRead"<<endl;
	__uint32_t &events_ = channel_->getEvents();
	do {
		bool zero = false;
		int read_num = readn(fd_, inBuffer_, zero);//experiment here
		LOG<<"request: "<<inBuffer_;
		//cout<<inBuffer_<<" "<<inBuffer_.size()<<endl;
		...
		if(state_ == STATE_RECV_BODY) {
			...
			{
				error_ = true;
				handleError(fd_, 400, "bad Request: lack of arg (content_length)");
				break;
			}
...
URIState HttpData::parseURI() {
	...
	string request_line = str.substr(0, pos);//without \r
	if(str.size() > pos +1)
	...
	cout<<"THERE!!!fileName_: "<<fileName_<<endl;
...
AnalysisState HttpData::analysisRequest() {
  if(method_ == METHOD_POST){
    ...
    if(fileName_ == "hello") {
		...
		header += "Server: WTMLON's Web Server\r\n";
		header += "\r\n";
		header += body;
		outBuffer_ = header;
/*	outBuffer_ =
...
void HttpData::handleError(int fd, int err_num, string short_msg) {
  ...
  body_buff += "<html><title>ouch! Something Wrong!</title>";
...
void HttpData::handleClose()
{
	...
	//above: out of range release autoly
